#!/usr/bin/env python
"""Recopilar datos de tipo de espacio de trabajo y registros de cambios

Las imágenes se recopilan de acuerdo con dos reglas:

1. el archivo se nombra en sintaxis de imagen visual de Markdown (de la forma ![...](...)),

2. para cualquier archivo seleccionado a través de la regla 1, si su nombre comienza con un ancho
   prefijo (por ejemplo, `480px-`), luego se elimina el prefijo para obtener un nuevo
...
if not os.path.exists(md_output_path):
    ...
    for p in Path(md_output_path).glob('**/*'):
        assert p.name.endswith('.md'), f'archivo inesperado: {str(p)}'
        p.unlink()
...
if not os.path.exists(img_output_path):
    ...
    for p in Path(img_output_path).glob('**/*'):
        assert p.suffix in accepted_image_file_suffixes, f'archivo inesperadoado: {str(p)}'
        p.unlink()
...
index = '''
# Índice de tipos de espacios de trabajo

(generado a partir de[https://github.com/rerobots/workspaces](https://github.com/rerobots/workspaces))
